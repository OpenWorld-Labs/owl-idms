...
from ..nn.resnet import ConvBlock
...
class ControlPredictor(nn.Module):
    def __init__(self, config : 'ControlPredConfig'):
        ...
        self.mouse_token = nn.Parameter(torch.randn(1, self.t_config.d_model)*0.02)
        self.button_token = nn.Parameter(torch.randn(1, self.t_config.d_model)*0.02)
        self.n_cls_toks = 2

        self.proj_t_in = nn.Linear(self.r_config.ch_max, self.t_config.d_model)
        self.pos_enc = LearnedPosEnc(8*4*4, self.t_config.d_model)

        n_per_block = 1
        total_blocks = 5

        def get_block(ch_in, ch_out):
            return ConvBlock(ch_in, ch_out)
        ...
        self.m_head_mu = MLPSimple(self.t_config.d_model, self.config.n_mouse_axes)
        self.m_head_logvar = MLPSimple(self.t_config.d_model, self.config.n_mouse_axes)
        self.b_head = MLPSimple(self.t_config.d_model, self.config.n_buttons)

        self.final_norm = LayerNorm(self.t_config.d_model)
    ...
    def forward(self, x):
        ...
        m_tok = self.mouse_token.unsqueeze(0).repeat(b,1,1)
        b_tok = self.button_token.unsqueeze(0).repeat(b,1,1)
        ...
        m_mu_preds = self.m_head_mu(m_emb) # -> [b,n_mouse_axes]
        m_logvar_preds = self.m_head_logvar(m_emb) # -> [b,n_mouse_axes]
        b_logits = self.b_head(b_emb) # -> [b,n_buttons]

        return (m_mu_preds, m_logvar_preds), b_logits
