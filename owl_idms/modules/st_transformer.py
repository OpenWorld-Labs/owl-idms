...
3. Lightweight projection (``Conv1×1``) + learned positional encoding.
...
class STInverseDynamics(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # 2. CLS tokens
        self.mouse_cls = nn.Parameter(torch.randn(1, 1, 1,      d_model))
        self.key_cls   = nn.Parameter(torch.randn(1, 1, n_keys, d_model))
        n_cls_tokens = self.mouse_cls.shape[2] + self.key_cls.shape[2]
        # 3. Positional encodings (learned) – separate spatial & temporal
        self.temporal_pe = nn.Parameter(torch.randn(1, frames, 1, d_model))
        self.spatial_pe  = nn.Parameter(torch.randn(1, 1, self.S + n_cls_tokens, d_model))
    ...
    # ------------------------------------------------------------------
    def forward(self, video: Tensor) -> ActionPrediction:
        ...
        # add CLS per frame
        mouse_tok = self.mouse_cls.expand(B, T, 1, -1)
        key_tok   = self.key_cls  .expand(B, T, self.n_keys, -1)
        x = torch.cat([mouse_tok, key_tok, x], dim=2)      # [B,T,S+2,C]
