...
class GroupNorm3d(nn.Module):
    ...
    def forward(self, x):
        b, c, t, h, w = x.shape
        x = x.reshape(b, self.groups, c // self.groups, t, h, w)

        var = x.float().var(dim=(2,3,4,5), keepdim=True, unbiased=False)
        std = (var + 1.0e-6).sqrt()
        x = x / std.to(x.dtype)

        x = x.reshape(b, c, t, h, w)
        return x * (1. + self.gain[None,:,None,None,None])
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        # Expand gain to match x's shape, with channel dim last
        # e.g. for 4D input: [1,1,1,d], for 3D: [1,1,d]
        gain_shape = [1] * (x.ndim-1) + [x.shape[-1]]
        gain = self.gain.view(*gain_shape)
